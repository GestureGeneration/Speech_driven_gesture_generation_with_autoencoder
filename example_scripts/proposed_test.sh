...
for seq in `seq 1 1 2`; 
		...
		echo
                ...
                CUDA_VISIBLE_DEVICES=$GPU python predict.py $model.hdf5 /home/taras/Documents/Datasets/SpeechToMotion/Irish/processed/$folder/test_inputs/X_test_Audio_${seq}.npy enc_${dim}_prediction$seq.txt
                mv enc_${dim}_prediction$seq.txt motion_repr_learning/ae/
                cd motion_repr_learning/ae/
                # Step2: Decode representation into motion
                CUDA_VISIBLE_DEVICES=$GPU python decode.py /home/taras/Documents/Datasets/SpeechToMotion/Irish/processed/$folder enc_${dim}_prediction${seq}.txt ../../example_scripts/gestures/gesture${seq}.txt -restore=True -pretrain=False -layer1_width=$dim -chkpt_dir='/home/taras/tmp/MoCap/'$dim -batch_size=8
...
mv example_scripts/gestures/*.txt evaluation/data/predicted/$speech_features/
cd evaluation
